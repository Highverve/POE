...
{
    public class SmelterEmbers : BaseEmitter
    {
        public SmelterEmbers() : base(-1, 0, 0)
        {
        ...
        {
            values = new ParticleValues(cm.Load<Texture2D>("Particles/white"), new Vector2(22, 12), .25f, 0, 2, 1000);
        ...
        protected override void ApplyRandomValues()
        {
            ...
            values.velocity.Y = (float)trueRandom.Next(-35, 5);
            values.velocity.X = (float)trueRandom.Next(-25, 25);

            base.ApplyRandomValues();
        }
        private Color orangeHalf = Color.Lerp(Color.Orange, Color.Transparent, .35f);
        protected override void ApplyParticleBehavior(GameTime gt, int i)
        {
            ...
            particles[i].Color = Color.Lerp(Color.Transparent, orangeHalf, (float)particles[i].TimeLeft / 1000);
